...
problem_data.h = @(x, y, ind) cos( theta( x , y ) ) * ( isempty( find( ind == problem_data.drchlt_sides_inhomogeneous ) ) == 0  ) + 0 * x .* y  ;
problem_data_0.h = @(x, y, ind) 0 * x .* y  ;
problem_data_0.h_auxiliar = @( x , y , ind ) cos( theta( x , y ) )  * ( isempty( find( ind == [problem_data_0.gamma_sides{:}] ) ) == 0  ) + 0 * x .* y  ;
...
problem_data_0.singularity_type = 'cos' ;
...
for ii = 1:nn
    ...
    A_fourier = epsilon/ ( epsilon ^ 2 - 1 ) ; 
    problem_data_0.alpha_dirac= 2 * pi * A_fourier;
    % 1) BUILD GEOMETRY
    ...
    if Find_alpha_opt_Dirac
        problem_data.u_0_exact = @( x , y ,  alpha ) alpha * x .* ( 1 - 1 ./ ( x .^ 2 + y.^ 2 ) )/(2*pi);
        [~, boundaries, ~, ~, ~] = mp_geo_load (problem_data.geo_name);
        gamma_side= problem_data.gamma_sides ; 
        meas_gamma = 4 * epsilon ; 
        l2_norm_bdry = @( alpha_vect ) get_l2_norm_boundary(boundaries,space,msh,gamma_side,problem_data,alpha_vect);
