...
r = @( x , y ) sqrt( x .^ 2 + y .^ 2 );
theta = @( x , y ) atan2( y , x );
problem_data.f = @(x, y) 0 * 2 * r( x , y ) .* cos( theta( x , y ) ); 
[problem_data, problem_data_0] = determineBC(problem_data);
problem_data.h = @(x, y, ind) ( isempty( find( ind == problem_data.drchlt_sides_inhomogeneous ) ) == 0 ) + 0 * x .* y   ;
problem_data_0.h = @(x, y, ind) 0 * x .* y ;
problem_data_0.h_auxiliar = @(x, y, ind) ( isempty( find( ind == [problem_data_0.gamma_sides{:}] ) ) == 0  ) + 0 * x .* y  ;
...
if plotIt
    fig = figure;
    loglog(eps_values, errh1s , 'o-r' , 'LineWidth' , 7,  'MarkerSize', 30);
    hold on 
    loglog(  eps_values, est, 'b' , 'Marker', '*' , 'LineWidth' , 7,  'MarkerSize', 30 )
    loglog( eps_values , est_h1_tg , '+-' ,'LineWidth' , 7,  'MarkerSize', 30   )
    % loglog( eps_values, eps_values, 'k:' , 'LineWidth' , 5 )
    % loglog( eps_values , 1 ./ sqrt( abs( log(eps_values) ) ) , '+-' , 'LineWidth' , 5 )
    grid on
    legend('$|u-u_0|_{1,\Omega}$', '$\mathcal{E}_{N}(u_0)$', '$\mathcal{E}_{N}(u_0,\nabla_{t}u_0)$' , 'Interpreter' , 'Latex' , 'Location', 'northwest' , 'Fontsize' , 100)
    xlabel( '$\epsilon$' , 'Interpreter','latex' , 'FontSize', 30 )
    set(gca,'FontSize',70)
    title(['$g_{d}=0\:on\:\gamma\:\:\:\:f=2rcos(\theta)\:in\:\Omega\:\:\:\:Shape:\:square$'] , 'Interpreter','latex' , 'FontSize', 70 ...
    )
