...
if plotIt
    fig = figure;
    loglog(eps_values, errh1s, '+-r' , 'LineWidth' , 7,  'MarkerSize', 30);
    hold on 
    loglog(  eps_values, est, '+-b' , 'LineWidth' , 7,  'MarkerSize', 30)
    loglog( eps_values , est_h1_tg , '+-' , 'LineWidth' , 7,  'MarkerSize', 30  )
    loglog( eps_values, eps_values, 'k:' , 'LineWidth' , 7,  'MarkerSize', 30 )
    grid on
    legend('$|u-u_0|_{1,\Omega}$', '$\mathcal{E}_{N}(u_0)$', '$\mathcal{E}_{N}(u_0,\nabla_{t}u_0)$' ,'$\epsilon$' , 'Interpreter' , 'Latex' , 'Location', 'northwest' , 'Fontsize' , 70 )
    xlabel( '$\epsilon$' , 'Interpreter','latex' , 'FontSize',90 )
    set(gca,'FontSize',30)
    title(['$g_{d}=0\:\gamma\:\:\:\:f=2rcos(\theta)\:\Omega\:\:\:\:Shape:square$'] , 'Interpreter','latex' , 'FontSize', 70 ...
    )
end
