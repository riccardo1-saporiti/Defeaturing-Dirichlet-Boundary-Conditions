% OP_U_V_TRIMMING: assemble the matrix M = [m(i,j)], m(i,j) = (epsilon u_j, v_i), for a trimmed geometry.
%
%   mat = op_u_v_trimming (spu, spv, msh_trimmed, [epsilon]);
%   [rows, cols, values] = op_u_v_trimming (spu, spv, msh_trimmed, [epsilon]);
%
% INPUT:
%
...
function varargout = op_u_v_trimming(space1, space2, msh_trimmed, coeff)
...
sp1 = sp_evaluate_element_list(space1.space_untrimmed, msh_elems, 'value', true);
sp2 = sp_evaluate_element_list(space2.space_untrimmed, msh_elems, 'value', true);
...
if(~isempty(msh_trimmed.reparam.non_trimmed_elem_ids))  
    ...
    coeffs = coeff (x{:});
    A_loc = op_u_v (sp1, sp2, msh_elems, coeffs); % assembling of non-trimmed elements
    A = A + A_loc(space2.active_dofs, space1.active_dofs);
end

% Assembly of trimmed elements
if (~isempty(msh_trimmed.reparam.trimmed_elems))
  A = A + op_u_v_tiles(space1, space2, msh_trimmed, coeff); 
end
