...
% INPUTS:
%     
%     space:      the coarse space, an object of the sp_trimming class (see sp_trimming)
%     msh:        the refined mesh, an object of the msh_trimming class (see msh_trimming)
...
% OUTPUT:
%
%     sp_fine: the refined space, an object of the class sp_trimming (see sp_trimming)
...
%
% Copyright (C) 2015, 2016, 2021 Rafael Vazquez
%
...
function [sp_fine, Proj] = sp_refine (space, msh, nsub, degree, regularity)
  ...
  if (any (degree < space.space_untrimmed.degree))
  ...
  if (strcmpi (space.space_untrimmed.space_type, 'spline'))
    knots = kntrefine (space.space_untrimmed.knots, nsub-1, degree, regularity);
    sp_fine = sp_trimming (knots, degree, msh);

    if (nargout == 2)
      if (all (sp_fine.space_untrimmed.degree == space.space_untrimmed.degree))
        for idim = 1:msh.ndim
          knt_coarse = space.space_untrimmed.knots{idim};
          knt_fine = sp_fine.space_untrimmed.knots{idim};
